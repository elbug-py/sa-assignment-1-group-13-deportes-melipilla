...
def review_create(request, book_id):
    ...
    if request.method == "POST":
        try:
            ...
            review.save()
            messages.success(request, f"Reseña agregada exitosamente al libro '{book.name}'.")
    ...
    if request.method == "POST":
        try:
            ...
            if existing_sale:
                ...
                existing_sale.save()
                messages.success(request, f"Se agregaron {count_int} ventas al año {year_int}. Total: {existing_sale.count} ventas.")
            else:
                # Crear nueva venta
                sale = Sale()
                sale.book = book
                ...
                sale.save()
                messages.success(request, f"Se agregaron {count_int} ventas para el año {year_int}.")
